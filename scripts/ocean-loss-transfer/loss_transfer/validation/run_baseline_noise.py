...
from loss_transfer.common.paths import PROJECT_ROOT, SANDBOX_CONFIG_DIR, SANDBOX_DIR, SCRIPTS_ROOT, TRAINING_PIPELINE_DIR

if str(SCRIPTS_ROOT) not in sys.path:
    sys.path.insert(0, str(SCRIPTS_ROOT))
from python_manager import find_first_python_path

from loss_transfer.common._types import BaselineThresholds
from loss_transfer.common._utils import parse_training_events
from loss_transfer.formula.sandbox_adapter_bridge import write_config_with_adapter

_PYTHON = sys.executable or find_first_python_path() or 'python3'

_PROJECT_ROOT = PROJECT_ROOT
_PIPELINE_DIR = TRAINING_PIPELINE_DIR
_SANDBOX_CONFIG_DIR = SANDBOX_CONFIG_DIR
_MODEL_CONFIG_MAP = {
    'swinir': 'swinir.yaml',
    'edsr': 'edsr.yaml',
    'fno2d': 'fno2d.yaml',
...
def _baseline_log_dir(model: str, run_idx: int) -> str:
    return str(SANDBOX_DIR / 'runs' / f'baseline_{model}_{run_idx}')

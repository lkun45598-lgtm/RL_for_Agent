...
@changelog
  ...
  - 2026-03-28 kongzhiquan: v1.4.0 restore patchable subprocess wrapper, zero-mask smoke validation, and timeout artifact export
  - 2026-03-28 kongzhiquan: v1.4.1 write timeout training-curve artifacts to a persistent temp file instead of the candidate-loss directory
"""
...
from loss_transfer.common.paths import PROJECT_ROOT, SANDBOX_DIR, SCRIPTS_ROOT, TRAINING_PIPELINE_DIR

if str(SCRIPTS_ROOT) not in sys.path:
    sys.path.insert(0, str(SCRIPTS_ROOT))
from python_manager import find_first_python_path, find_python_with_module
from loss_transfer.formula.runtime_routing import (
    FULL_RUN_MODEL_CONFIGS,
    needs_temporary_runtime_config,
    probe_model_output_extension_support,
)
from loss_transfer.formula.sandbox_adapter_bridge import (
...
from loss_transfer.common._utils import parse_training_events

def _resolve_runtime_python() -> str:
    current = (sys.executable or '').strip()
    if current and Path(current).is_file():
        return current
    return find_python_with_module('torch') or find_first_python_path() or 'python3'


_PYTHON = _resolve_runtime_python()
...
_PIPELINE_DIR = TRAINING_PIPELINE_DIR
...
def _write_training_curve_artifact(
    *,
    loss_file_path: str,
    training_curve: Dict[str, Any],
) -> Dict[str, str]:
    fd, artifact_path_str = tempfile.mkstemp(
        prefix=f'{Path(loss_file_path).stem}_training_curve_',
        suffix='.json',
    )
    os.close(fd)
    ...
    return {
        'training_curve_path': str(artifact_path),
    }
...
def _resolve_sandbox_override_dir(loss_file_path: str) -> Optional[str]:
    override_dir = Path(loss_file_path).resolve().parent / 'sandbox_overrides'
    if override_dir.is_dir():
        return str(override_dir)
    return None


def _probe_override_module_import(
    *,
    module_name: str,
    override_dir: Path,
) -> ValidationResult:
    env = _build_run_env(extra_env={'SANDBOX_OVERRIDE_DIR': str(override_dir)})
    probe_code = (
        ...
        "override_dir = os.environ.get('SANDBOX_OVERRIDE_DIR', '')\n"
        'if override_dir:\n'
    ...
    try:
        result = subprocess.run(
            [sys.executable, '-c', probe_code, module_name, str(_PROJECT_ROOT)],
            cwd=_PROJECT_ROOT,
            env=env,
            capture_output=True,
            text=True,
        ...
        return {
            'passed': False,
            'error': 'override_import_timeout',
            'detail': f'sandbox override import probe timed out for module {module_name}',
        }
    ...
    if result.returncode != 0:
        ...
        return {
            'passed': False,
            'error': 'override_import_failed',
            'detail': f'failed to import override module {module_name}: {stderr[-500:]}',
        }

    resolved_file = (result.stdout or '').strip().splitlines()
    return {
        'passed': True,
        'module': module_name,
        'resolved_file': resolved_file[-1] if resolved_file else '',
    }


def _validate_sandbox_overrides(loss_file_path: str) -> ValidationResult:
    override_dir_str = _resolve_sandbox_override_dir(loss_file_path)
    if not override_dir_str:
        return {'passed': True}

    override_dir = Path(override_dir_str)
    checked_files: List[str] = []
    for py_file in sorted(override_dir.rglob('*.py')):
        ...
        try:
            ...
            return {
                'passed': False,
                'error': 'override_syntax_error',
                'detail': (
                    f'sandbox override file {py_file.relative_to(override_dir)} failed to compile: '
                    f'{exc.msg or exc}'
                ),
            }
        checked_files.append(str(py_file.relative_to(override_dir)))

    checked_modules: List[Dict[str, Any]] = []
    module_candidates = []
    if (override_dir / 'sandbox_model_adapter.py').is_file():
        module_candidates.append('sandbox_model_adapter')
    if (override_dir / 'sandbox_trainer.py').is_file():
        module_candidates.append('sandbox_trainer')
    if (override_dir / 'models' / '__init__.py').is_file():
        module_candidates.append('models')

    for module_name in module_candidates:
        probe = _probe_override_module_import(
        ...
        checked_modules.append(
            {
                'module': module_name,
                'resolved_file': probe.get('resolved_file'),
            }
        )
...
def validate_static(loss_file_path: str) -> ValidationResult:
    ...
    # 8. attempt-scoped sandbox override 静态/import 校验
    override_result = _validate_sandbox_overrides(loss_file_path)
    if not override_result.get('passed'):
    ...
    sandbox_overrides = override_result.get('sandbox_overrides')
    if isinstance(sandbox_overrides, dict):
        result['sandbox_overrides'] = sandbox_overrides
    return result
...
def validate_single_model(
    ...
    with tempfile.TemporaryDirectory(prefix='sandbox_single_') as temp_dir:
        ...
        combined_output = str(run_result.get('combined_output') or '')

        if run_result.get('timed_out'):
            training_curve = parse_training_events(combined_output)
            partial_metrics: TrainingMetrics = {}
            valid_epochs = _collect_valid_epochs(training_curve)
            if valid_epochs:
                last = valid_epochs[-1]
                partial_metrics = {
                    'val_ssim': float(last.get('ssim', 0) or 0),
                    'val_psnr': float(last.get('psnr', 0) or 0),
                }

            artifact_paths = _write_training_curve_artifact(
            ...
            if training_curve.get('last_epoch', -1) >= 0:
                ...
                if valid_epochs:
                    last_ssim = valid_epochs[-1].get('ssim')
                    if isinstance(last_ssim, (int, float)):
                        detail += f'，最后 SSIM={float(last_ssim):.4f}'
        ...
        try:
            if valid_epochs:
                ...
                ssim_match = re.search(r'val_ssim:\s+([\d.]+)', combined_output)
                psnr_match = re.search(r'val_psnr:\s+([\d.]+)', combined_output)
...
def validate_smoke(loss_file_path: str, formula_spec_path: Optional[str] = None) -> ValidationResult:
    ...
    try:
        ...
        # 测试 zero-mask 输出是否保持在计算图上
        pred_zero_mask = pred.detach().clone().requires_grad_(True)
        target_zero_mask = target.detach().clone()
        zero_mask = torch.zeros_like(mask)
        zero_mask_kwargs = build_smoke_loss_kwargs(formula_spec, pred_zero_mask)
        zero_mask_loss = sandbox_loss(pred_zero_mask, target_zero_mask, mask=zero_mask, **zero_mask_kwargs)

        if not isinstance(zero_mask_loss, torch.Tensor) or zero_mask_loss.dim() != 0:
            return {
                'passed': False,
                'error': 'invalid_zero_mask_output',
                'detail': 'zero-mask 路径必须返回标量 Tensor',
            }

        if torch.isnan(zero_mask_loss) or torch.isinf(zero_mask_loss):
            return {
                'passed': False,
                'error': 'invalid_zero_mask_output',
                'detail': 'zero-mask 路径返回 NaN/Inf',
            }

        if not zero_mask_loss.requires_grad:
            return {
                'passed': False,
                'error': 'detached_zero_mask_loss',
                'detail': 'zero-mask 路径返回的 loss 没有连接到 pred 的计算图',
            }

        pred_zero_mask.grad = None
        zero_mask_loss.backward()
        if pred_zero_mask.grad is None:
            return {
                'passed': False,
                'error': 'detached_zero_mask_loss',
                'detail': 'zero-mask 路径 backward 后没有产生梯度',
            }
...
def validate_full_run(
    ...
    for model_name in FULL_RUN_MODEL_CONFIGS:
    ...
    with tempfile.TemporaryDirectory(prefix='sandbox_full_') as temp_dir:
        if needs_temporary_runtime_config(formula_spec, dataset_root=dataset_root):
            for config_name in FULL_RUN_MODEL_CONFIGS.values():
        ...
        # 改进6: 超时恢复 - 从已写入的日志文件解析部分结果
            partial_metrics: TrainingMetrics = {}
            for model in FULL_RUN_MODEL_CONFIGS:
        ...
        if baseline_thresholds:
            viable_threshold = baseline_thresholds.get('viable_threshold')
            if not isinstance(viable_threshold, (int, float)):
                baseline_center = baseline_thresholds.get('ssim_mean', baseline_thresholds.get('swinir_mean', 0.6645))
                viable_threshold = float(baseline_center) - 0.01
            swinir_ssim = metrics.get('swinir', 0)  # type: ignore[arg-type]
            if swinir_ssim < viable_threshold:

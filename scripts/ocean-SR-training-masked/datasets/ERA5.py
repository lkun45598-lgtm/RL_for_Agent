#single channel ERA5 dataset (u or v or tempterature component)
import torch
...
class ERA5Dataset:
    def __init__(self, data_args, **kwargs):        
        ...
        if osp.exists(process_path):
            ...
            # Load ERA5 data from .npy file
            # Expected shape: [B, 512, 512, 1] where channels are u or v or temperature components
    ...
    def pre_process(self, data, mode='train', normalize=False, 
        ...
        # data shape: (B, H, W, C) where C=1 (u or v component)
        B, H, W, C = data.shape
...
class ERA5Base(Dataset):
    """
    A base class for the ERA5 dataset.
    ...
    Attributes:
        ...
        x (torch.Tensor): The low-resolution input data with u and v temperature components.
        y (torch.Tensor): The high-resolution target data with u and v temperature components.
    """
